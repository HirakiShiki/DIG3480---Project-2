...
Level.prototype.isFinished = function() {      // New, checks if the level was cleared.
...
function Player(pos) {
  this.pos = pos.plus(new Vector(0, -1.5)); // "-1.5" makes sure that the player doesn't spawn in the ground, since they're 50% taller than the base.
  this.size = new Vector(1.2, 2.5);
...
function Fireball(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.8, 0.4));
...
DOMDisplay.prototype.drawFrame = function() {
  ...
  this.wrap.className = "game " + (this.level.status || "");  // New.
...
DOMDisplay.prototype.clear = function() {   // New.
  this.wrap.parentNode.removeChild(this.wrap);   // New.
};
...
Level.prototype.actorAt = function(actor) {
  ...
  for (var i = 0; i < this.actors.length; i++) {
    ...
    if (other != actor &&
        actor.pos.x + actor.size.x > other.pos.x && //Checks if the player (or actor) is overlapping within another actor on the X-Axis.
        actor.pos.x < other.pos.x + other.size.x && //Checks if the player (or actor) is overlapping AND inside the bounding box of the other actor on the X-Axis.
...
Level.prototype.animate = function(step, keys) {
  ...
  if (this.status != null) // New.
    this.finishDelay -= step; // New.
...
// New, everything commented out in this block.
...
var fireballwobbleSpeed = 7, fireballwobbleDist = 4.0; //Modify

Fireball.prototype.act = function(step) {
  this.wobble += step * fireballwobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * fireballwobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
var gravity = 30;
var jumpSpeed = 17;
var playerYSpeed = 9; //Might need to be removed? (Added to code from Lab 6)
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle == "lava") {
      level.status = "lost"; // New.
      level.finishDelay = 1; // New.
      // this.pos = resetPos;
    }
...
Player.prototype.act = function(step, level, keys) {
  ...
  // Losing animation
  if (level.status == "lost") { // New.
    this.pos.y += step; // New.
    this.size.y -= step; // New.
  }
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type == "lava" && this.status == null) {  // New.
    this.status = "lost"; // New.
    this.finishDelay = 1; // New.
  }
    ...
    if (!this.actors.some(function(actor) { // New.
           return actor.type == "coin"; // New.
         })) { // New.
      this.status = "won"; // New.
      this.finishDelay = 1; // New.
    }
...
function runLevel(level, Display, andThen) {
  ...
  runAnimation(function(step) {
    ...
    if (level.isFinished()) {  // New.
      display.clear();  // New.
      if (andThen)  // New.
        andThen(level.status);  // New.
      return false;  // New.
    }
  });
}
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")  // New.
        startLevel(n);  // New.
      else if (n < plans.length - 1)  { // New.
        startLevel(n + 1);
        // document.getElementById("background").style.backgroundImage = "url('images/Stage 6.gif')"  // New.
      }
      else  // New.
        console.log("You win!");  // New.
    });
  }
  startLevel(0);
}
